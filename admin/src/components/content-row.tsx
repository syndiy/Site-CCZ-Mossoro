...
import { Check, FileText, GripVertical, Image as ImageIcon, Newspaper, Plus } from "lucide-react";
...
export function ContentRow({
  ...
  const Icon = entry.collection === "news" ? Newspaper : FileText;
  ...
  return (
    ...
    >
      <span className="drag-handle" title="Arrastar para ordenar"><GripVertical size={18} /></span>
      <span className="order-number">{position}</span>
      <div className="row-cover">{entry.cover ? <img src={entry.cover} alt="" /> : <Icon size={18} />}</div>
      ...
      <button className={`home-toggle ${entry.home ? "selected" : ""}`} type="button" onClick={() => onToggle(entry)} aria-label={entry.home ? "Retirar da pagina inicial" : "Colocar na pagina inicial"} title={entry.home ? "Retirar da pagina inicial" : "Colocar na pagina inicial"}>
        {entry.home ? <Check size={17} /> : <Plus size={17} />}
      </button>
