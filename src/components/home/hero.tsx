...
export function Hero({ eyebrow, title, subtitle, image, imageAlt, slides, children }: HeroProps) {
  ...
  const chamada = (
    <Container className={comSlides ? "relative pt-12 md:pt-16" : "relative py-16"}>
  ...
  if (!comSlides) {
    return (
      <section className="relative flex min-h-[560px] items-center overflow-hidden bg-brand-900 md:min-h-[640px]">
        ...
        />
        <div className="relative z-20 w-full">{chamada}</div>
      </section>
    );
  }

  // A chamada e a legenda dividem a mesma coluna: nenhuma das duas é absoluta,
